...
import com.example.newsify.util.Constants.Companion.QUERY_PAGE_SIZE
...
class BreakingNewsFragment : Fragment(R.layout.fragment_breaking_news) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        breaking_fab_goToTop.setOnClickListener {
            rv_breaking_news.smoothScrollToPosition(0)
        }

        val snapHelper = PagerSnapHelper()
        snapHelper.attachToRecyclerView(rv_breaking_news)
            ...
            when (response) {
                is Resource.Success -> {
                    ...
                    response.data?.let { newsResponse ->
                        ...
                        if (isLastPage) {
                            paginationProgressBar.setPadding(0, 0, 0, 0)
                        }
                    }
                }
                is Resource.Error -> {
